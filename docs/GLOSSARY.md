...
| :--- | :--- | :--- |
| **Share Key** | **共有キー** | ❌ シェアキー, ❌ スラッグ, ❌ Share key (日本語環境下) |
| **Space** | **スペース** | ❌ ルーム, ❌ 会場 |
| **Dashboard** | **ダッシュボード** | 「管理画面」よりも「ダッシュボード」を優先 |
| **Host / Owner** | **主催者** または **所有者** | スペース作成者は「所有者」で統一 |
| **Participant** | **参加者** | ❌ ゲスト, ❌ ユーザー |
| **Join** | **参加する** | ❌ 入室する |

**重要**: 
- 内部実装（変数名、DBカラム名、関数名等）では `slug` や `share_key` を使用可能
...
- **コード上の命名**:
  ...
  - フォームフィールド: `slug`（内部実装のみ）
  - Zod スキーマ: `spaceSchema.slug`（内部実装のみ）
- **形式**: `<user-input>-<YYYYMMDD>` （例: `my-party-20251224`）
- **制約**: 小文字の英数字とハイフンのみ、3〜30文字
- **UI表示ルール**:
  - ✅ 正しい表記: 「共有キー」
  - ❌ 使用禁止: 「シェアキー」「スラッグ」「slug」「Share key」（日本語環境下）
  - 内部変数名やDBカラム名は `slug` / `share_key` を使用可能だが、ユーザーの目に触れる翻訳ファイルやUIラベルでは必ず「共有キー」を使用すること
...
| 日付       | 変更内容                                                       | 変更者         |
| ---------- | -------------------------------------------------------------- | -------------- |
| 2025-12-30 | UI表示における用語統一ルールを追加、Share Key の定義を明確化 | GitHub Copilot |

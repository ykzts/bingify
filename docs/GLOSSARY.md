...
| **Gatekeeper**         | **ゲートキーパー** または **アクセス制御** | 参加条件の設定機能                                                                                   |
...
Bingifyは**Supabase Authを利用したユーザー認証・ロール管理システム**を実装しています。OAuth (Google、Twitch) によるログインが必須であり、認証されたユーザーのみがスペースの作成や管理をできます。
...
- **定義**: スペースを作成したユーザー。ビンゴの抽選をしてスペースを管理する
...
- **定義**: Next.jsのServer Actions。サーバーサイド実行の関数であり、フォーム送信やデータ処理に使用される

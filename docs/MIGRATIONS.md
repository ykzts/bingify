...
3. PR に明確な説明と、なぜ既存ファイルを編集する必要があるのかの理由を記載する
...
## ソーシャルメタデータテーブル（20260109130000）

Issue #520 で実装されたソーシャル連携のメタデータ管理テーブルについて説明します。

### 概要

スペースのゲートキーパールールにおいて、YouTube チャンネルや Twitch ブロードキャスターの ID だけでなく、ユーザーフレンドリーな情報（ハンドル名、チャンネル名など）も保存・表示できるようにするため、以下の 2 つのマスターテーブルを追加しました：

- `youtube_channels`: YouTube チャンネルのメタデータ
- `twitch_broadcasters`: Twitch ブロードキャスターのメタデータ
...
|---------|---|------|
| id | UUID | 主キー |
| channel_id | TEXT | YouTube チャンネル ID（UC で始まる 24 文字、UNIQUE） |
| handle | TEXT | YouTube ハンドル（@username 形式） |
| channel_title | TEXT | チャンネル名 |
| custom_url | TEXT | カスタム URL |
| thumbnail_url | TEXT | サムネイル画像の URL |
| description | TEXT | チャンネルの説明 |
| fetched_at | TIMESTAMP | メタデータの最終取得日時 |
| fetch_error | TEXT | API 取得時のエラーメッセージ |
...
|---------|---|------|
| id | UUID | 主キー |
| broadcaster_id | TEXT | Twitch ブロードキャスター ID（数字のみ、UNIQUE） |
| username | TEXT | Twitch ユーザー名（小文字） |
| display_name | TEXT | Twitch 表示名（大文字小文字混在可） |
| profile_image_url | TEXT | プロフィール画像の URL |
| description | TEXT | プロフィールの説明 |
| fetched_at | TIMESTAMP | メタデータの最終取得日時 |
| fetch_error | TEXT | API 取得時のエラーメッセージ |
| created_by | UUID | 作成者（auth.users への外部キー） |
| created_at | TIMESTAMP | レコード作成日時 |
...
- `gatekeeper_rules`（spaces テーブルの JSONB カラム）には引き続き ID のみを保持
- メタデータは専用のマスターテーブルで管理
- 複数スペース間でのデータ共有とキャッシングが可能

#### キャッシング戦略

- API 取得から 24 時間以内のデータはキャッシュとして再利用
- `fetched_at` カラムで最終取得日時を記録
- API 呼び出しの削減によるコスト削減とパフォーマンス向上

#### エラーハンドリング

- API 取得失敗時は `fetch_error` カラムにエラーメッセージを保存
- キャッシュされたデータがある場合は、古いデータでもエラーよりも優先して返す
- UI では ID のみをフォールバック表示

### Row Level Security (RLS)

- **読み取り**: 全ユーザーが可能（認証不要）
- **挿入・更新**: 認証済みユーザーのみ可能
- **削除**: ポリシーなし（データは永続的にキャッシュ）

### ライフサイクル管理

スペースは最長 1 日程度の短命なリソースであるため、孤立レコードのクリーンアップは基本的に不要です：

- チャンネル/ブロードキャスター情報を永続的にキャッシュ
- 将来の再利用で即座に利用可能
- オプションで 6 ヶ月以上参照されていないレコードの定期削除も可能（未実装）
...
} from "@/lib/data/youtube-metadata";
import {
  fetchAndCacheTwitchBroadcasterMetadata,
  formatTwitchBroadcasterDisplay,
} from "@/lib/data/twitch-metadata";

// YouTube チャンネルメタデータの取得
const youtubeMetadata = await fetchAndCacheYouTubeChannelMetadata(
  supabase,
  channelId,
  accessToken,
  userId
...
// 表示用文字列の生成: "@handle (UCxxxxxx...)" または "Channel Name (UCxxxxxx...)"
const displayText = formatYouTubeChannelDisplay(youtubeMetadata);

// Twitch ブロードキャスターメタデータの取得
const twitchMetadata = await fetchAndCacheTwitchBroadcasterMetadata(
  supabase,
  broadcasterId,
  appAccessToken,
  userId

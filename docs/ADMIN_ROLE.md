...
### 方法1: セットアップウィザードを使用（推奨）

システムに管理者が存在しない場合、最初の認証済みユーザーがセットアップウィザードを使用して管理者権限を取得できます。

1. アプリケーションにサインイン
2. `/setup`または`/admin`に移動（管理者が存在しない場合、自動的に`/setup`にリダイレクト）
3. 「管理者権限を取得」ボタンをクリック
...
- 認証が必須です - 未認証ユーザーはセットアップページにアクセスできません

### 方法2: SQLを使用（ローカル開発）

**注意:** セットアップウィザード（方法1）がより安全で簡単なため、このマニュアル方法は管理者が既に存在する場合にのみ使用してください。
...
5. **ビルド時とランタイムの要件**
   - セットアップウィザードと管理者ページは`dynamic = "force-dynamic"`でマークされています
   - これによりSupabase認証情報はビルド時には不要で、実行時にのみ必要になります
   - CI/CD環境でビルドする際、Supabaseインスタンスが動作している必要はありません
   - 環境変数は実行時（デプロイメント環境）でのみ必要です

-- Fix upsert_oauth_token_for_user() function to correctly handle vault.update_secret()
-- This migration addresses the same issue as 20260108121739 but for the service_role function
-- vault.update_secret() returns void (not uuid), so we should use PERFORM and keep the existing UUID
  ...
  WHEN OTHERS THEN
    ...
    BEGIN
      IF v_created_access AND v_access_secret_id IS NOT NULL THEN
        ...
        IF v_delete_result IS NULL THEN
          RAISE WARNING 'Failed to cleanup access token secret id=% during exception handling for user_id=% provider=%: %', v_access_secret_id, p_user_id, p_provider, SQLERRM;
        END IF;
      END IF;
      ...
      IF v_created_refresh AND v_refresh_secret_id IS NOT NULL THEN
        ...
        IF v_delete_result IS NULL THEN
          RAISE WARNING 'Failed to cleanup refresh token secret id=% during exception handling for user_id=% provider=%: %', v_refresh_secret_id, p_user_id, p_provider, SQLERRM;
        END IF;
      END IF;
    EXCEPTION
      WHEN OTHERS THEN
        -- Log cleanup errors but don't fail the outer exception
        RAISE WARNING 'Cleanup failed for user_id=% provider=% access_secret_id=% refresh_secret_id=%: %', p_user_id, p_provider, v_access_secret_id, v_refresh_secret_id, SQLERRM;
    END;
...
COMMENT ON FUNCTION public.upsert_oauth_token_for_user(UUID, TEXT, TEXT, TEXT, TIMESTAMPTZ) IS 'Upserts an OAuth token for a specific user (service_role only). Fixed to correctly handle vault.update_secret() which returns void (not uuid), and to validate secret existence before updates.';

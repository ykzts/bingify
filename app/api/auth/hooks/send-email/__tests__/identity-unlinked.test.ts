...
describe("Identity Unlinked Handler", () => {
  describe("handleEmailAction - identity_unlinked_notification", () => {
    it("英語ロケールでメール送信する", async () => {
      const { sendAuthEmail } = await import("@/lib/mail");
      vi.mocked(sendAuthEmail).mockClear();

      const emailData: EmailData = {
        email_action_type: "identity_unlinked_notification",
        factor_type: "",
        old_email: "",
        old_phone: "",
        provider: "Google",
      ...
      expect(sendAuthEmail).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          subject: "External Provider Unlinked",
        })
      );
    });

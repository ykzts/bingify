...
複数のサービスを統合して管理する場合は、Docker Compose を使用できます。
...
`.env.production` を編集して、実際の環境変数を設定してください。
...
- **postgres**: PostgreSQL データベース (ポート 5432) - オプション
- **nginx**: リバースプロキシ (ポート 80/443) - オプション、デフォルトはコメントアウト

### nginx リバースプロキシの有効化

nginx を使用する場合は、`docker-compose.yml` 内の nginx セクションのコメントを解除してください。
...
- `nginx/conf.d/default.conf`: バーチャルホスト設定

SSL証明書を使用する場合は、`nginx/ssl/` ディレクトリに証明書を配置し、`nginx/conf.d/default.conf` の SSL 設定のコメントを解除してください。
...
- **Supabase**: Supabase Cloud の使用を推奨します。セルフホストの場合は `postgres` サービスを有効にしてください。
- **SSL証明書**: Let's Encrypt などの証明書を使用し、HTTPS を有効にしてください。
- **環境変数**: `.env.production` ファイルは `.gitignore` に含まれています。秘密情報を含めないでください。
- **バックアップ**: データベースボリューム (`postgres-data`) の定期的なバックアップを設定してください。
